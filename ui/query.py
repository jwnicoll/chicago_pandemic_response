...
TABLE_DICT = {'percent_cases': ['tests_deaths_vacs_zip AS tdvz'],
        ...
        'test_sites': ['test_sites AS sites'],
        ...
        'test_site_access': ['location AS l2', 'location AS l1', 'test_sites AS sites'],
...
JOIN_CONDS = {'icu_accessibility': ['hosp.Zip_Code == l1.Zip_Code'],
        ...
        'test_site_access': ['sites.Zip_Code == location.Zip_Code'],
        'deaths_adjusted': ['tdvz.Zip_Code == ages.Zip_Code']}


def find_columns(args):
    '''
    Takes in a dictionary with two keys, one corresponding to the first
    ...
    info and returns a list of tuples where each tuple is a row (Zip_Code) and
    each entry in the tuple is the value of interest to be plotted on the heatmaps
    '''

    arg1, arg2 = args.values()
